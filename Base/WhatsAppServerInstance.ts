...
//05-09-25 01:20  AJE: Fixed TypeScript error with msgRetryCounterCache using CacheStore adapter - v6.7.18.112
...
const APP_VERSION = '6.7.18.112';
const BUILD_DATE = '05-09-25 01:20';
const VERSION_DESCRIPTION = 'Enhanced GUID + WhatsApp ID duplicate control + Fixed TypeScript cache compatibility';
...
// 05-09-25 01:20 - AJE: Create CacheStore adapter for NodeCache to fix TypeScript compatibility
...
async function removeAuthFolder(){
    ...
    // 05-09-25 00:40 - AJE: Mostrar información detallada de rutas para debugging
    console.log(`🔍 Directorio de trabajo actual: ${process.cwd()}`);
    console.log(`🔍 __dirname: ${__dirname}`);
    console.log(`🔍 Total de rutas a verificar: ${authFolders.length}`);
    
    for (const folder of authFolders) {
        const absolutePath = path.resolve(folder);
        console.log(`🔍 Verificando carpeta: ${folder} -> ${absolutePath}`);
...
async function removeSession() {
    ...
    // 05-09-25 00:50 - AJE: Corregir rutas - archivos están DENTRO de baileys_auth_info
    if (instanceId && instanceId !== '') {
        const cwd = process.cwd();
        
        // Los archivos están DENTRO de la carpeta baileys_auth_info
        files.push(
            // Dentro de baileys_auth_info en el directorio actual
            path.join(cwd, 'baileys_auth_info', 'auth_info_multi.json'),
            path.join(cwd, 'baileys_auth_info', 'statesLog.json'),
            
            // Rutas absolutas correctas - DENTRO de baileys_auth_info
            `C:\\WhatsAppInstances\\${instanceId}\\Baileys\\baileys_auth_info\\auth_info_multi.json`,
            `C:\\WhatsAppInstances\\${instanceId}\\Baileys\\baileys_auth_info\\statesLog.json`,
            `D:\\WhatsAppInstances\\${instanceId}\\Baileys\\baileys_auth_info\\auth_info_multi.json`,
            `D:\\WhatsAppInstances\\${instanceId}\\Baileys\\baileys_auth_info\\statesLog.json`,
            
            // También buscar directamente (por si acaso)
        ...
        console.log(`📂 Directorio base: ${cwd}`);
        console.log(`📄 NOTA: Archivos deben estar en baileys_auth_info/`);
    }

    // 05-09-25 00:40 - AJE: Mostrar información detallada para debugging
    console.log(`🔍 Total de archivos a verificar: ${files.length}`);
    ...
    for (const filePath of files) {
        try {
            ...
            console.log(`🔍 Verificando archivo: ${filePath} -> ${absolutePath}`);
            if (fs.existsSync(absolutePath)) {
